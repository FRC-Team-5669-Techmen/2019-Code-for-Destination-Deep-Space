...
void loop() {
  ...
  for (byte sensorIndex = 0; sensorIndex < NUM_SENSORS; sensorIndex++) {
    ...
    if (!ACTIVE[sensorIndex]) continue;
  ...
  for (uint sensorIndex = 0; sensorIndex < NUM_SENSORS; sensorIndex++) {
    if (!ACTIVE[sensorIndex]) continue;
    if (values[sensorIndex] <= maxValue) {
      uint weight = (maxValue - values[sensorIndex]) / (range / OUTPUT_PRECISION);
      totalWeight += weight;
